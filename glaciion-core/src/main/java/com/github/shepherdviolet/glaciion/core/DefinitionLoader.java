...
class DefinitionLoader {
    ...
    static {
        //parse exclusions
        String exclusions = System.getProperty(VMOPT_EXCLUDE_FILE, null);
        if (exclusions != null) {
            //split by ,
    ...
    static List<InterfaceDefinition> loadInterfacesDefinitions(ClassLoader classLoader, final String loaderId) {
        ...
        loadFiles(PATH_INTERFACES, classLoader, loaderId, new DefinitionVisitor() {
            @Override
    ...
    static List<SingleDefinition> loadSingleDefinitions(final String interfaceType, ClassLoader classLoader, final String loaderId){
        ...
        loadFiles(PATH_SINGLE_SERVICE + interfaceType, classLoader, loaderId, new DefinitionVisitor() {
            @Override
            public void visitFileStart(String url) {
            }
            @Override
            public void visitDefinition(String name, String value, String url) {
                ...
                if (CommonUtils.isEmpty(name)) {
                    throw new IllegalDefinitionException(loaderId + " | Illegal single-service definition, find a property with empty key, correct format is 'sample.SampleServiceImpl -1', url:" + url + ", see:" + LOG_HOME_PAGE, null);
                }
                //parse priority
                int priority = 0;
                if (!CommonUtils.isEmpty(value)) {
                    try {
                        ...
                        throw new IllegalDefinitionException(loaderId + " | Error while parsing priority of single-service, at " + name + " " + value + ", url:" + url + ", see:" + LOG_HOME_PAGE, e);
                    }
                }
                //add
    ...
    static List<MultipleDefinition> loadMultipleDefinitions(final String interfaceType, ClassLoader classLoader, final String loaderId){
        ...
        loadFiles(PATH_MULTIPLE_SERVICE + interfaceType, classLoader, loaderId, new DefinitionVisitor() {
            @Override
            public void visitFileStart(String url) {
            }
            @Override
            public void visitDefinition(String name, String value, String url) {
                //check name
                if (CommonUtils.isEmpty(name)) {
                    throw new IllegalDefinitionException(loaderId + " | Illegal multiple-service definition, find a property with empty key, correct format is '+sample.SampleServiceImpl' or '-sample.SampleServiceImpl' (+/- can be multiple), url:" + url + ", see:" + LOG_HOME_PAGE, null);
                }
                ...
                if (c == '+') {
                    ...
                    for (int i = 1 ; i < name.length() ; i++) {
                        ...
                        if (c == '+') {
                            ...
                            throw new IllegalDefinitionException(loaderId + " | Illegal multiple-service definition, when the first char is +, it must be followed by +, correct format is '++sample.SampleServiceImpl' (+ can be multiple), url:" + url + ", see:" + LOG_HOME_PAGE, null);
                        } else {
                            break;
                        }
                    }
                    ...
                    throw new IllegalDefinitionException(loaderId + " | Illegal multiple-service definition, a property is not starts with +/-, correct format is '+sample.SampleServiceImpl' or '-sample.SampleServiceImpl' (+/- can be multiple), url:" + url + ", see:" + LOG_HOME_PAGE, null);
                }
                if (rank > name.length() - 1) {
                    throw new IllegalDefinitionException(loaderId + " | Illegal multiple-service definition, property key cannot only have +/- signs, correct format is '+sample.SampleServiceImpl' or '-sample.SampleServiceImpl' (+/- can be multiple), url:" + url + ", see:" + LOG_HOME_PAGE, null);
                }
                //add
    ...
    static List<PropertiesDefinition> loadPropertiesDefinitions(final String bindType, ClassLoader classLoader, final String loaderId){
        ...
        loadFiles(PATH_PROPERTIES + bindType, classLoader, loaderId, new DefinitionVisitor() {
            ...
            public void visitFileEnd() {
                //parse priority
                String priorityStr = definition.getProperties().remove(PROPERTY_PRIORITY);
                if (priorityStr != null) {
                    try {
                        definition.setPriority(Integer.parseInt(priorityStr));
                    } catch (Exception e) {
                        throw new IllegalDefinitionException(loaderId + " | Error while parsing " + PROPERTY_PRIORITY + " of properties file, url:" + definition.getUrl() + ", see:" + LOG_HOME_PAGE, e);
                    }
                }
                //add definition
            ...
            try {
                ...
                if (FILE_EXCLUSIONS.size() > 0) {
                    ...
                    if (FILE_EXCLUSIONS.contains(hash)) {
                        LOGGER.warn(loaderId + " | Exclude file " + url + " by -D" + VMOPT_EXCLUDE_FILE, null);
                        continue;
                    }
                }
                //parse file
